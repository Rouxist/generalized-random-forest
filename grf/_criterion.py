...
class GRFCriterion:
    ...
    def get_theta_p_hat(self, y_parent) -> float:                                       # pylint: disable=C0116
    ...
    def get_psi(self, y_c: list, theta_hat_p: float) -> np.ndarray:                     # pylint: disable=C0116
        # Depends on Regression equation
        return np.array([np.mean([y_c - theta_hat_p])])

    def get_psi_vec(self, y_c: list, theta_hat_p: float) -> np.ndarray:                 # pylint: disable=C0116
        # Depends on Regression equation
        return np.array(y_c) - theta_hat_p

    def get_xi(self) -> np.ndarray:                                                     # pylint: disable=C0116
        # Depends on Regression equation
        return np.array([1])

    def get_a_p(self, y_c: list, theta_hat_p: float) -> np.ndarray:                     # pylint: disable=C0116, W0613
        # Depends on Regression equation
        return np.array([[-1]])

    def get_inv_a_p(self, a_p: np.ndarray) -> np.ndarray:                               # pylint: disable=C0116
        if len(a_p) == 1:
            return 1/a_p
        else:
            return inv(a_p)

    def get_proxy_delta_tilde(self, y_left:np.ndarray, y_right:np.ndarray) -> float:    # pylint: disable=C0116
        ...
        theta_p_hat = self.get_theta_p_hat(y_parent=y_parent)
        # for y_i = b + u_i, it equals to mean of y_parent
