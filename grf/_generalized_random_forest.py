...
class GRF:
    ...
    self.fit()
        It divides given dataset into two subsets to satisfy hoensty condition 
        and fits multiple gradient trees on bootstrapped subsets.
    ...
    def __init__(self, target:str, n_estimators:int=5, min_samples_leaf:int=3,
                 max_depth:int=5, honest:bool=True, data_weight_ratio:float = 0.5,
                 max_samples:float=.45, block_size:int=1, random_state:int=None) -> None:
        ...
        self.data_weight_ratio = data_weight_ratio        # ratio of given dataset to be used when calculating                              pylint: disable=C0301
                                                          # weights for local estimation  << deprecated                                     pylint: disable=C0301
        self.min_samples_leaf = min_samples_leaf          # minimum numbers of datapoints
                                                          # required for a new leaf when splitting
        ...
        self.max_samples = max_samples                    # each tree is trained on this portion of whole train data.                       pylint: disable=C0301
        self.block_size = block_size                      # block size to be used as a parameter of block sampling.                         pylint: disable=C0301
    ...
    def fit(self, data:pd.DataFrame) -> None:   # pylint: disable=R0914
        """
        Fits the model
        """
        self.data_columns = [self.target]+['X' + str(i+1) for i in range(1, data.shape[1])] # when dependent variable has dimension of 1    pylint: disable=C0301
        self.subsample_random_state_seed = self.random_state.randint(MAX_INT) # `self.subsample_random_seed_` of econml _base_grf.py        pylint: disable=C0301
        subsample_random_state = np.random.RandomState(self.subsample_random_state_seed) # pylint: disable=no-member
        ...
        for estimator_indices in estimator_idx_groups:
            for _ in range(len(estimator_indices)):
                block_start_indices = subsample_random_state.choice(n_samples - self.block_size + 1, n_blocks, replace=False) # pylint: disable=C0301
                block_sampled_data = []
                for start_idx in block_start_indices:
                    block_sampled_data.extend([i for i in range(start_idx, start_idx + self.block_size)]) # pylint: disable=C0301
    ...
    def predict(self, x:pd.DataFrame|pd.Series) -> float:
        ...
        """
        data_weight_list = [self.tree_list[i].data_parent.iloc[self.tree_list[i].indices_val]
        ...
        if n_given_datapoints == 1:          # pylint: disable=R1705
            estimate_given_dps = [tree.predict(is_own_weight_set=False, X=x)     # length of n_estimator and each element has    pylint: disable=C0301
                                  for tree in self.tree_list]                    # length of given datapoints                    pylint: disable=C0301
            for tree_idx, tree in enumerate(self.tree_list):
                ...
                indices, preds = tree.predict(is_own_weight_set=True,
                                              col_names=x.columns)
                prediction_pairs = zip(indices, preds)
                neighbors = [idx for (idx, weight_data_estim)
                             in prediction_pairs
                             if estimate_given_dps[tree_idx][0]==weight_data_estim]

                # Update weights
                if len(neighbors) > 0:
                    for neighbor_idx in neighbors:
                        self.alpha[0][whole_data_weight_indices.index(neighbor_idx)] += 1 / len(neighbors) / self.n_estimators # pylint: disable=C0301
            ...
            estimate_given_dps = [tree.predict(is_own_weight_set=False, X=x)   # length of n_estimator and each element   pylint: disable=C0301
                                  for tree in self.tree_list]                  # has length of given datapoints           pylint: disable=C0301
            ...
            for dp_idx in range(n_given_datapoints):
                ...
                for tree_idx, tree in enumerate(self.tree_list):
                    ...
                    neighbors = [idx for (idx, weight_data_estim)
                                 in prediction_pairs
                                 if estimate_given_dps[tree_idx][dp_idx]==weight_data_estim]

                    # Update weights
                    if len(neighbors) > 0:
                        for neighbor_idx in neighbors:
                            self.alpha[dp_idx][whole_data_weight_indices.index(neighbor_idx)] += 1 / len(neighbors) / self.n_estimators # pylint: disable=C0301
