...
BLOCK_SIZE = 4 # 1 if when not using block sampling
...
### Generating u_t
extended_T = T + N_EXTRA_ERROR_TERM           # Total number of T required including     # pylint: disable=C0103
...
### Testing point
beta_1_test_x_random_state = np.random.RandomState(BETA_1_TESTING_POINT_SEED) # pylint: disable=no-member
beta_1_test_x = beta_1_test_x_random_state.multivariate_normal(mean=np.zeros(N_FEATURES),
                                                               cov=cov_matrix, size=1)
beta_1_test_x = pd.DataFrame(beta_1_test_x, columns=['X' + str(i+1) for i in range(N_FEATURES)])

beta_2_test_x_random_state = np.random.RandomState(BETA_2_TESTING_POINT_SEED) # pylint: disable=no-member
beta_2_test_x = beta_2_test_x_random_state.multivariate_normal(mean=np.zeros(N_FEATURES),
                                                               cov=cov_matrix, size=1)
...
if __name__ == "__main__":
    ...
    time_now_date = time.strftime('%b', time_now) + " " \
                  + time.strftime('%d', time_now) + " " \
                  + time.strftime('%Y', time_now)
    time_now_time = time.strftime('%H', time_now) + ":" \
                  + time.strftime('%M', time_now) + ":" \
                  + time.strftime('%S', time_now)
    ...
    print("{:<30}: {:>10}  {:<30}: {:>10}".format("Seed for beta1",
                                                  BETA_1_TESTING_POINT_SEED,
                                                  "Seed for beta2", 
                                                  BETA_2_TESTING_POINT_SEED))
    ...
    print("{:<30}: {:>10}  {:<30}: {:>10}".format("No. initial u_t truncated",
                                                  N_EXTRA_ERROR_TERM,
                                                  "No. independent variable",
                                                  N_FEATURES))
    print()
    print("[Random Forest setup]")
    print("{:<30}: {:>10}  {:<30}: {:>10}".format("Block size", BLOCK_SIZE,
                                                  "No. estiamtor", N_ESTIMATORS))
    print("======================================================================================")
    print("\n")

    x_axis = list(range(len(X[:,0])))
    plt.plot(x_axis, y)
    plt.tight_layout()
    plt.savefig(f"./result/data__rho_{RHO}.png")

    if SHOW_DATA:
        print("                                     Generated Data                                   ") # pylint: disable=C0301
        print("======================================================================================") # pylint: disable=C0301
        ...
        print("\n")
    if SHOW_T_VAR:
        print("                                   Treatment Variable                                 ") # pylint: disable=C0301
        print("======================================================================================") # pylint: disable=C0301
    ...
    if ECONML_MODEL:
        ...
        print("                                   Experiment Result                                  ") # pylint: disable=C0301
        print("======================================================================================") # pylint: disable=C0301
        print("{:<25}: {:>15}  {:<25}: {:>15}".format("Date", time_now_date,
                                                      "Time", time_now_zone + " " + time_now_time))
        print("{:<25}: {:>15}  {:<25}: {:>11.2f} sec".format("Model", "Scratch",
                                                             "Time taken", time_taken))
        print()
        print("{:<25}: {:>15.5}  {:<25}: {:>15.5}".format("Mean of beta1 hat",
                                                          np.mean(arr_estimated_beta_1),
                                                          "Std. dev. of beta1 hat", 
                                                          np.std(arr_estimated_beta_1)))
        print("{:<25}: {:>15.5}  {:<25}: {:>15.5}".format("Mean of beta2 hat",
                                                          np.mean(arr_estimated_beta_2),
                                                          "Std. dev. of beta2 hat", 
                                                          np.std(arr_estimated_beta_2)))
        print("======================================================================================") # pylint: disable=C0301
        print("\n")
        ...
        if SHOW_PLOT:
            ...
            axes[0].set_title('Histogram of beta1')
            axes[1].hist(arr_estimated_beta_2, bins=50)
            axes[1].set_title('Histogram of beta2')
    ...
    if FROM_SCRATCH_MODEL:
        ...
        print("                                   Experiment Result                                  ") # pylint: disable=C0301
        print("======================================================================================") # pylint: disable=C0301
        print("{:<25}: {:>15}  {:<25}: {:>15}".format("Date",
                                                      time_now_date,
                                                      "Time", 
                                                      time_now_zone + " " + time_now_time))
        print("{:<25}: {:>15}  {:<25}: {:>11.2f} sec".format("Model", "Scratch",
                                                             "Time taken", time_taken))
        print()
        print("{:<25}: {:>15.5}  {:<25}: {:>15.5}".format("Mean of beta1 hat",
                                                          ...
                                                          "Std. dev. of beta1 hat",
                                                          np.std(arr_estimated_beta_1)))
        print("{:<25}: {:>15.5}  {:<25}: {:>15.5f}".format("Mean of beta2 hat",
                                                           np.mean(arr_estimated_beta_2),
                                                           "Std. dev. of beta2 hat",
                                                           np.std(arr_estimated_beta_2)))
        print("======================================================================================") # pylint: disable=C0301
...
# To see data given to each tree
print("whole data:\n",df.iloc[:,:6])
for tree in grf_scratch.tree_list:
    print(f"Data of tree{tree.idx}:\n{tree.data_parent.iloc[:,:6]}\n")
